...
2. 从根节点开始，逐层创建层级
3. 使用 `outline_create_node` 添加每个节点
4. 设置节点类型（act/sequence/scene/beat）

### 修改结构
1. 使用 `outline_get_structure` 获取完整树
2. 使用 `outline_move_node` 调整节点位置
3. 使用 `outline_update_node` 更新节点属性

### 关联实体
- 使用 `outline_link_entity` 将实体绑定到指定节点
- 支持角色/区域/物品/事件等所有类型
...
| `outline_update_node` | 更新节点属性 |
| `outline_move_node` | 移动节点位置 |
| `outline_get_structure` | 获取大纲树结构 |
| `outline_link_entity` | 关联实体到节点 |
