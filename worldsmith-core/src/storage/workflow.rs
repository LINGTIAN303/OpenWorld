...
impl<'a> WorkflowStore<'a> {
    ...
    pub fn save(&self, def: &WorkflowDefinition) -> Result<(), CoreError> {
        ...
        let json = serde_json::to_string(def)
            .map_err(|e| CoreError::storage(format!("序列化失败: {e}")))?;
        self.conn
            .execute(
    ...
    pub fn list(
        ...
        let rows = stmt
            ...
            .map_err(|e| CoreError::storage(e.to_string()))?;
        rows.into_iter()
            .map(|r| r.map_err(|e| CoreError::storage(e.to_string())))
            .collect()
    }
    ...
    pub fn append_node_log(
        ...
        let payload_json = match payload {
            Some(v) => Some(
                serde_json::to_string(&v).map_err(|e| CoreError::storage(e.to_string()))?,
            ),
            None => None,
        };
        let now = now_ms();
    ...
    pub fn remove_pending_dispatch(&self, request_id: &str) -> Result<(), CoreError> {
        self.conn
            .execute(
                "DELETE FROM workflow_pending_dispatches WHERE request_id = ?1",
                params![request_id],
            )
    ...
    /// 序列化为 JSON/YAML
    pub fn serialize(&self, def: &WorkflowDefinition, format: ParseFormat) -> Result<String, CoreError> {
