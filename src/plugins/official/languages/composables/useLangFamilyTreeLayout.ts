import type { LangNode, LangEdge } from './useLangFamilyTreeData'

export type LangLayoutMode = 'top-down' | 'radial'

export function useLangFamilyTreeLayout() {
  function layoutTopDown(nodes: LangNode[], edges: LangEdge[]): void {
    if (nodes.length === 0) return

    const branchEdges = edges.filter(e => e.type === 'branch')
    const relatedEdges = edges.filter(e => e.type === 'related' && e.relation === '祖先语言')
    ...
    const layerGroups = new Map<number, LangNode[]>()
    for (const n of nodes) {
    ...
    for (const [, layerNodes] of layerGroups) {
      layerNodes.sort((a, b) => {
        const fa = a.languageFamily || ''
        const fb = b.languageFamily || ''
        if (fa !== fb) return fa.localeCompare(fb)
        return a.langType.localeCompare(b.langType)
      })
    }
  ...
  function layoutRadial(nodes: LangNode[], edges: LangEdge[]): void {
    if (nodes.length === 0) return
    ...
    const ringGroups = new Map<number, LangNode[]>()
    for (const n of nodes) {
  ...
  function applyLayout(nodes: LangNode[], edges: LangEdge[], mode: LangLayoutMode): void {
    if (mode === 'top-down') {
