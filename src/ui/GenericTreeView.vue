...
function onDragOver(e: DragEvent) {
  ...
  if (!expanded.value && props.node.children.length > 0) {
    if (!dragTimer) {
      dragTimer = setTimeout(() => { expanded.value = true; dragTimer = null }, 500)
    }
  }
}
...
function onDrop(e: DragEvent) {
  ...
  if (draggedId && draggedId !== props.node.entity.id) {
    ...
    if (!isDesc) {
      emit('move-node', draggedId, props.node.entity.id, dropPosition.value === 'before' ? -1 : dropPosition.value === 'after' ? 1 : 0)
    }
  }
}
