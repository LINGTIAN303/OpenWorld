...
function crossProduct(a: { x: number; y: number }, b: { x: number; y: number }, c: { x: number; y: number }): number {
...
function onMouseMove(e: MouseEvent) {
  ...
  if (!dragging) {
    ...
    canvasRef.value!.style.cursor = hit && mode.value === 'edit' ? 'pointer' : mode.value === 'draw' ? 'crosshair' : 'grab'
    if (hit) {
      const r = hit.region
